      ...
      <div className="space-y-4 mb-6">
        ...
        */}
        ...
        {invoice.settlementContext === 'AFTER_EXPIRY' && (
          <div className="bg-amber-50 border border-amber-300 rounded-lg p-4 text-amber-900 text-sm" role="alert">
            <p className="font-semibold mb-1">Settlement Notice: Payment Received After Expiry</p>
            <p>
              Payment of {formatAmount(invoice.amount, 7)} {invoice.assetCode} was received on {formatDate(invoice.settledAt || invoice.paidAt || invoice.createdAt)} after invoice expiry ({formatDate(invoice.expiresAt)}). Recorded as late settlement.
            </p>
          </div>
        )}

        {invoice.settlementContext === 'AFTER_CANCEL' && (
          <div className="bg-amber-50 border border-amber-300 rounded-lg p-4 text-amber-900 text-sm" role="alert">
            <p className="font-semibold mb-1">Settlement Notice: Payment Received After Cancellation</p>
            <p>
              Payment of {formatAmount(invoice.amount, 7)} {invoice.assetCode} was received on {formatDate(invoice.settledAt || invoice.paidAt || invoice.createdAt)} after invoice cancellation ({formatDate(invoice.cancelledAt || invoice.createdAt)}). Recorded as post-cancellation settlement.
            </p>
          </div>
        )}
      ...
      <div className="border-t pt-6 space-y-3 print:hidden">
        ...
        */}
        <button
          ...
          aria-label={canEmail && invoice.customerEmail ? `Email payment proof to ${invoice.customerEmail}` : 'Email Proof'}
