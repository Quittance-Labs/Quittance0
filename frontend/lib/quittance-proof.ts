...
/**
 * Validates a candidate proof document against the canonical schema rules.
 *
 * @param doc - Document to validate against schema.
 * @returns SchemaValidationResult with valid boolean and list of error messages.
 */
export function validateQuittanceProofSchema(doc: unknown): SchemaValidationResult {
  ...
  for (const key of Object.keys(obj)) {
    if (!QUITTANCE_PROOF_FIELDS.includes(key as QuittanceProofField)) {
      errors.push(`Disallowed extra field: ${key}`);
    }
  }
  ...
  if (typeof obj.seller !== 'string' || !stellarPubKeyRegex.test(obj.seller)) {
    errors.push('seller must be a valid Stellar public key (56 characters, starts with G)');
  }

  if (obj.payer !== null && (typeof obj.payer !== 'string' || !stellarPubKeyRegex.test(obj.payer))) {
  ...
  if (!payment || typeof payment !== 'object' || Array.isArray(payment)) {
    ...
    if (typeof payment.amount !== 'string' || !/^\d+(?:\.\d{1,7})?$/.test(payment.amount)) {
      errors.push('payment.amount must be a string formatted decimal with up to 7 places');
    }
  ...
  if (!verification || typeof verification !== 'object' || Array.isArray(verification)) {
    ...
    if (verification.checkedAt !== null && (typeof verification.checkedAt !== 'string' || !dateRegex.test(verification.checkedAt))) {
