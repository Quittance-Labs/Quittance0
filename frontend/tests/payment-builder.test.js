...
test('isValidStellarPublicKey checks Stellar public key formatting', () => {
  ...
  assert.equal(isValidStellarPublicKey('SA7QYNF7SOWQ3GLR2BGMZEHXAVIRZA4KVWLTJJFC7MGXUA74P7UJVSGZ'), false); // S prefix is secret key
  assert.equal(isValidStellarPublicKey('GA7QYNF7SOWQ3GLR2BGMZEHXAVIRZA4KVWLTJJFC7MGXUA74P7UJVSG'), false); // 55 chars
});
...
test('validatePaymentInvoice rejects zero, negative, or invalid amounts', () => {
  ...
  const tooManyDecimals = validatePaymentInvoice({
    destination: VALID_DESTINATION,
    amount: '10.12345678', // 8 decimals > Stellar 7 decimals
    memo: VALID_MEMO,
  });
...
test('validatePaymentInvoice enforces memo presence, max 28 bytes, and exact match', () => {
  ...
  // Exact memo match against invoice memo
...
test('classifyPaymentError distinguishes transport, wallet, and verification errors', () => {
  ...
  const wrongNet = classifyPaymentError(new Error('Switch Freighter to Testnet to create or pay invoices.'));
