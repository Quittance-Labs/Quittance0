/**
 * Focused tests for the pay verify and outage controller (issue #445).
 */
...
test('validateTxHash uses the canonical hash rules', () => {
...
test('executePaymentVerification handles Horizon outage via VERIFY_UNAVAILABLE and is retryable', async () => {
  const events = [];
...
test('executePaymentVerification handles backend rejection via VERIFY_FAILED', async () => {
  const events = [];
