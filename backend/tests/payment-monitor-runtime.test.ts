...
describe('PaymentMonitorService Runtime & Multi-Invoice Watch', () => {
  ...
  it('rejects cross-attribution when transaction hash was already claimed by another invoice', async () => {
    ...
    const mockInvoicesService = {
      ...
      async markAsPaid() {
        throw new PaymentClaimError('TRANSACTION_ALREADY_USED', 'Transaction already settled an invoice');
      },
      async markExpiredInvoices() {
        return 0;
      },
  ...
  it('handles replayed transaction for already-PAID invoice safely', async () => {
    ...
    const result = await monitor.runOnce();
    assert.equal(result.processed, 1);
    assert.equal(markAsPaidCalled, false, 'Should not attempt markAsPaid for replay with same hash');
    assert.equal(events.length, 0, 'No rejection event for identical replay');
  });
  ...
  it('survives process restart without duplicate processing across instances', async () => {
    ...
    // Second instance simulates process restart with same checkpoint store
    ...
    // Resumes at cursor 601, returns 0 new records, no double settlement
  ...
  it('wires watch registration and unregistration to invoice handler lifecycle', async () => {
    ...
    // 1. Test createInvoice registers watch
    const reqCreate: any = {
      body: {
        sellerPublicKey: SELLER_KEY,
        amount: 10,
    ...
    // 2. Test cancelInvoice unregisters watch
    const reqCancel: any = {
    ...
    // 3. Test simulatePayment (or verifyPayment) unregisters watch
  ...
  it('serves payment monitor status route with watchedCount, processedTotal, and lagSeconds', async () => {
    ...
    const monitor = new PaymentMonitorService({
      account: SELLER_KEY,
      checkpoints: new TestCheckpointStore('700'),
      database: { query: async () => ({ rows: [], rowCount: 1 }) } as any,
    });
    ...
    assert.ok(route, 'GET /payment/monitor/status route exists');
